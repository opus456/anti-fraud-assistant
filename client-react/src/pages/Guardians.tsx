    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
        <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
          <UserPlus className="w-5 h-5 text-green-600" /> 绑定监护人
        </h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-sm">暂未绑定监护人</p>
            <p className="text-gray-400 text-xs mt-1">在上方输入监护人用户名进行绑定</p>
          </div>
        ) : (
          <div className="space-y-3">
            {guardians.map((g) => (
              <div key={g.id} className="flex items-center gap-4 p-4 border rounded-lg hover:bg-gray-50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-800">{g.guardian_nickname || g.guardian_username}</p>
                  <p className="text-xs text-gray-500">关系: {g.relationship || '未设置'} {g.is_primary ? '(主监护人)' : ''}</p>
                ...
                <button onClick={() => handleUnbind(g.id, g.guardian_nickname || g.guardian_username)}
                  className="px-3 py-1.5 text-sm text-red-600 bg-red-50 hover:bg-red-100 rounded-lg flex items-center gap-1 shrink-0">
