...
FRAUD_KEYWORDS = {
    "investment": {
        "keywords": [
            ...
            # 增强：投资诈骗高频词
            "内部投资", "投资计划", "年化", "回报率", "分红", "返利",
            "日收益", "周收益", "月收益", "稳定收益", "专业理财",
        ],
        "weight": 0.85,
    },
    "impersonation": {
        "keywords": [
            ...
            # 增强：社保/医保异常诈骗关键词
            "社保卡", "医保卡", "社保异常", "医保异常", "停用", "失效",
            "违规操作", "违法记录", "工作人员", "指定APP", "远程核查",
        ],
        "weight": 0.9,
    },
    "romance": {
...
HIGH_RISK_PATTERNS = [
    ...
    {
        "name": "fake_friend_borrow_money",
        "pattern": r"(我是.{0,8}(啊|呀)|出了?.{0,4}(急事|点事)).{0,30}(借.{0,6}(钱|款)|周转|转.{0,6}(到|给).{0,10}(卡号|账号|账户))",
        "weight": 0.52,
    },
    {
        "name": "urgent_transfer_promise",
        "pattern": r"(急用|急需|救急|周转).{0,20}(明天|回头|马上|立刻).{0,10}(还|给|转)",
        "weight": 0.45,
    },
    # 新增：社保异常+下载APP+远程核查 模式
    {
        "name": "social_insurance_app_scam",
        "pattern": r"(社保|医保).{0,15}(异常|停用|违规|违法).{0,30}(工作人员|APP|远程)",
        "weight": 0.50,
    },
    # 新增：紧急通知+添加微信 模式
    {
        "name": "urgent_notice_wechat",
        "pattern": r"(紧急通知|立即处理).{0,30}(添加.{0,5}微信|微信.{0,5}xxx)",
        "weight": 0.45,
    },
    # 新增：投资计划+高收益 模式
    {
        "name": "investment_high_return",
        "pattern": r"(投资计划|内部.{0,5}(消息|投资)).{0,30}(年化|回报|收益).{0,10}(\d+%|翻倍)",
        "weight": 0.48,
    },
]
...
def calculate_text_risk_score(text: str) -> dict:
    ...
    # 规则1：关键词命中多个分类时，增加得分（跨类型诈骗特征更可疑）
    if len(fraud_scores) >= 2:
        total_score += 0.08
    
    # 规则2：关键词得分中等以上时，确保不会被误判为安全
    if keyword_score >= 0.4:
        total_score = max(total_score, 0.25)  # 保底超过阈值0.22
    
    # 规则3：紧迫词命中2个以上时，增强风险信号
    if len(urgent_signals) >= 2:
        total_score = max(total_score, total_score + 0.06)
    
    # 规则4：同时有紧迫词和关键词命中，这是典型诈骗组合
    if urgent_signals and keyword_score >= 0.3:
        total_score = max(total_score, 0.28)
